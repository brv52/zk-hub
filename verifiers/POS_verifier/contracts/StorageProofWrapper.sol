...
contract StorageProofWrapper is IUniversalVerifier {
    ...
    function verifyProof(
        ...
        (uint256 expectedStateRoot, uint256 expectedThreshold) = abi.decode(
            verifierConfig,
            (uint256, uint256)
        );
        ...
        require(
            decodedSignals.length == 5,
            "StorageProof: Invalid signals count"
        );
        ...
        require(
            proofStateRoot == expectedStateRoot,
            "StorageProof: Invalid State Root Snapshot"
        );
        ...
        require(
            isValid,
            "StorageProof: Cryptographic proof is invalid or balance too low"
        );
